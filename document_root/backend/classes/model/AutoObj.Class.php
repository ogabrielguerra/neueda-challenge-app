<?
	Class AutoObj extends ConnectionAdmin {
		...
		function getObjData($idOrFilter) : array{
			...
			if(!$this->fields)
				$this->fields = $this->Table->getCampos();
		...
		//Get num total items
		function getTotalItensObj(string $filter, string $tablesOverride="") : int{

			if($tablesOverride!=""){
                $tableTarget = $tablesOverride;
			}else{
			...
			$this->getDataByQuery(array($index), $tableTarget, $filter, false);
